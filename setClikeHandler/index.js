...
let parameters = {
	project: 'poi-cloud-config',
	path: '/Users/zw_mac/POI_DEVELOP/poi-cloud-config/src/components/lib',
	router_path: '/Users/zw_mac/POI_DEVELOP/poi-cloud-config/src/router/router_menu.ts',
...
const getRouterPath = (full_path) => {
    ...
    // console.log(122, path, now_path, full_path)
...
const setClickFunc = ({ html, this_path, path, nameArr }) => {
	...
	let new_html = html.replace(/@click/g, (str, num, temp) => {
        ...
        buriedPointArr.push({ path, type: btnTypeJson[nameArr[index - 1]], name: nameArr[index - 1] || '', id,  });
