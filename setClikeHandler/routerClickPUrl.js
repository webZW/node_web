...
const { public, config, operation, pretreat, view, service } = require('./routerList.js');
const click_url = require('./click_url.json');
const buriedPoint_0916 = require('./buriedPointRecord/firstBuriedPoint_0916/buriedPoint_service_0923.js');

const routerList = [ ...public, ...config, ...operation, ...pretreat, ...view, ...service ];

const buriedPoinds = [];
const errorPoinds = [];
buriedPoint_0916.map((configItem) => {
    const { id, name, type, path } = configItem;
    console.log(1122, path);
...
fs.writeFileSync(`./buriedPointRecord/firstBuriedPoint_0916/buriedPoint_service_0923_1.json`, JSON.stringify(buriedPoinds, null, 4));