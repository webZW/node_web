	...
	wmtsUtil.fn = wmtsUtil.prototype = {
			...
			var format = new GeoGlobe.Format.WMTSCapabilities();
		  	GeoGlobe.Request.GET({ 
	            ...
	            success: function(request){
					//为了解决BoundingBox或者WGS84BoundingBox中存在“NaN NaN”的情况，导致format.read解析时报错，出现这个问题时，将坐标改为广东省范围
					var responseText = request.responseText;
					if(responseText.indexOf("<ows:LowerCorner>NaN NaN</ows:LowerCorner>") >= 0 ||
						responseText.indexOf("<ows:UpperCorner>NaN NaN</ows:UpperCorner>") >= 0){
						responseText = responseText.replaceAll("<ows:LowerCorner>NaN NaN</ows:LowerCorner>", "<ows:LowerCorner>109.61 20</ows:LowerCorner>");
						responseText = responseText.replaceAll("<ows:UpperCorner>NaN NaN</ows:UpperCorner>", "<ows:UpperCorner>117.39 25.51</ows:UpperCorner>");
					}

            		var json = format.read(responseText);  // 版本号赋值
		...
		WMTSMatchAnalyzer: function(json){
			...
			$(layers).each(function(i,data){
				...
				if(data.bounds){
					layerObj["tileFullExtent"]=data.bounds.getWest()+","+data.bounds.getSouth()+","+data.bounds.getEast()+","+data.bounds.getNorth();
				}
